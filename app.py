...
# Flask-SQLAlchemy requiere una URI por defecto incluso usando BINDS
if not app.config.get("SQLALCHEMY_DATABASE_URI"):
    app.config["SQLALCHEMY_DATABASE_URI"] = app.config.get("SQLALCHEMY_DATABASE_URI_USERS")

# Configurar binds para las 2 bases de datos de AlwaysData
...
class Product(db.Model):
    ...
    def get_price_config(self):
        try:
            ...
            return {}
    def to_dict(self):
        return {"id": self.id, "store_id": self.store_id, "category": self.category, "subcategory": self.subcategory, "name": self.name, "description": self.description, "image_url": self.image_url, "stock": self.stock, "price_config": self.get_price_config(), "is_active": self.is_active}
...
def calculate_price(product_obj, selected):
    cfg = product_obj.get_price_config() if hasattr(product_obj, "get_price_config") else product_obj.get("price_config", {})
    ptype = cfg.get("type", "fixed")
    if ptype == "size":
        return float(cfg.get("prices", {}).get(selected.get("size", "pequeño"), 0))
    if ptype == "size_flavor":
        return float(cfg.get("prices", {}).get(selected.get("size", "pequeño"), 0))
    if ptype == "fixed_size_especial":
        return float(cfg.get("price", 60))
    if ptype == "bocadito_clasico":
        return float({"25":10,"50":20,"100":40}.get(str(selected.get("qty","25")),10))
    if ptype == "bocadito_empanada":
        qty = int(selected.get("qty",25))
        return float(max(1, qty//25)*20)
    if ptype == "fixed":
...
@app.route("/health")
def health():
    # endpoint para Render health check y diagnóstico
    try:
        # intenta conteo de tiendas
        cnt = Store.query.count()
        return jsonify({"status":"ok","stores":cnt, "binds": list(app.config["SQLALCHEMY_BINDS"].keys())})
    except Exception as e:
...
def api_cart_add():
    ...
    for item in cart:
        if item["product_id"]==product_id and item["variant_detail"]==variant_detail and item["unit_price"]==unit_price:
            ...
            break
    if not found:
        cart.append({"product_id": product.id, "store_id": product.store_id, "store_slug": product.store.slug if product.store else "", "store_name": product.store.name if product.store else "", "name": product.name, "image_url": product.image_url, "variant_detail": variant_detail, "unit_price": unit_price, "quantity": qty_units, "subtotal": round(unit_price*qty_units,2), "category": product.category})
    save_cart(cart)
...
def admin_create_product():
    ...
    try:
        p=Product(store_id=store.id, category=data.get("category"), subcategory=data.get("subcategory","general"), name=data.get("name"), description=data.get("description",""), image_url=data.get("image_url",""), stock=int(data.get("stock",100)), price_config=json.dumps(data.get("price_config",{})))
        db.session.add(p)
        db.session.commit()
...
def admin_api_orders():
    ...
    result=[]
    for o in orders:
        result.append({"id":o.id,"store":o.store.name if o.store else "","customer_name":o.customer_name,"dni":o.dni,"email":o.email,"phone":o.phone,"payment_method":o.payment_method,"total":o.total,"created_at":o.created_at.isoformat(),"status":o.status, "items":[{"product_name":i.product_name,"variant_detail":i.variant_detail,"quantity":i.quantity,"unit_price":i.unit_price,"subtotal":i.subtotal} for i in o.items]})
...
# ===================== GLOBAL ERROR HANDLERS (funcionan con gunicorn) =====================
@app.errorhandler(500)
def internal_error(error):
    tb = traceback.format_exc()
    print(f"[500] {error}\n{tb}")
    return f"<h1>Error Interno (500)</h1><p>{error}</p><pre style='background:#FFF8EF;padding:12px;border-radius:8px;overflow:auto'>{tb}</pre><p>Verifica DATABASE_URL_USERS y DATABASE_URL_ADMIN en Render Environment</p>", 500

@app.errorhandler(404)
def not_found_error(error):
    return "<h1>404 - No encontrado</h1><p>La página no existe</p>", 404
...
# Inicialización idempotente: se ejecuta en cada request la primera vez (compatible gunicorn)
@app.before_request
def ensure_init():
    if not getattr(app, "_seed_done", False):
        try:
            with app.app_context():
                seed_data()
            app._seed_done = True
            print("[init] seed_data ejecutado correctamente")
        except Exception as e:
            print(f"[init] seed_data falló: {e}")
            traceback.print_exc()
            # no marcar como hecho para reintentar
            app._seed_done = True

# Intento de init al importar (para gunicorn worker)
try:
    with app.app_context():
        db.create_all()
        print("[import] db.create_all() ok")
except Exception as e:
    print(f"[import] db.create_all falló (normal si DB no configurada aún): {e}")
